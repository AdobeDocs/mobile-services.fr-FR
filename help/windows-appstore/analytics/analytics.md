...
Pour être défini *`products`* dans le SDK mobile, vous devez utiliser une syntaxe spéciale. Voir Variable [](/help/windows-appstore/analytics/products/products.md)Produits.

## (Facultatif) Activation du suivi hors ligne {#section_955B2A03EB854742BDFC4A0A3C287009}

Pour stocker les accès lorsque le périphérique est hors ligne, vous pouvez activer le suivi hors ligne dans la configuration [](/help/windows-appstore/c-configuration/methods.md)ADBMobileConfig.json. Avant d’activer le suivi hors ligne, tenez compte des exigences d’horodatage décrites dans la référence de fichier de configuration.
...
* Latitude/Longitude et point d’accès (s’il s’agit d’un point d’accès défini dans le fichier de `ADBMobileConfig.json` configuration). Elles sont transmises aux variables de solution mobile pour le rapports automatique.
